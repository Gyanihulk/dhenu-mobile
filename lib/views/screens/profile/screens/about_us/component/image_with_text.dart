...
class ImageWithText extends StatelessWidget {
  ...
  final bool isImageLeft; // If true, image is on the left; otherwise, on the right.
  final Color? textBackgroundColor; // Optional background color for the text container
  ...
  const ImageWithText({
    ...
    this.title,
    this.fontColor, // New optional font color parameter
  });
      ...
      child: Row(
        ...
        children: [
          if (isImageLeft) _buildImage(context), // Image on the left
           Expanded(
            child: Padding(
              padding: const EdgeInsets.symmetric(vertical: 16.0), // Padding only for text
              child: _buildTextWithTitle(context),
            ),
          ),
  ...
  Widget _buildImage(BuildContext context) {
    ...
    final double imageWidth = textBackgroundColor != null
        ? MediaQuery.of(context).size.width *
            0.4 // Wider when background color is present
        : MediaQuery.of(context).size.width * 0.3;

    final double imageHeight = textBackgroundColor != null
        ? MediaQuery.of(context).size.width *
            0.7 // Adjust height when background color is present
        : MediaQuery.of(context).size.width * 0.3;

    final BorderRadius imageBorderRadius = textBackgroundColor != null
        ? BorderRadius.circular(
            0) // No rounded corners when background color is present
        : BorderRadius.circular(
            12.0); // Rounded corners when no background color
    ...
    return Container(
      ...
      child: ClipRRect(
        borderRadius:
            imageBorderRadius, // Apply conditional border radius to the image
        child: Image.asset(
          imagePath,
          fit: BoxFit.cover,
          width: imageWidth,
          ...
          if (title != null) ...[
            Text(
              ...
              style: Theme.of(context).textTheme.titleMedium?.copyWith(
                    ...
                    fontSize: 14,
                    color: fontColor ?? Colors.black, // Use fontColor if provided
                  ),
            ),
