...
class HomeRepository extends BaseRepository {
  ...
  try {
    ...
    final response = await requestHttps(
      RequestType.GET,
      '${ApiConstants.notificationsEndpoint}/$userId/notifications', // Add this endpoint to ApiConstants
      null, // No body required for GET requests
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['data']['data'] != null && responseData['data']['data'] is List) {
      ...
      throw Exception(
          'Failed to fetch notifications: ${response.statusCode} ${response.reasonPhrase}');
    }
  } catch (error) {
    log('Error in fetchNotifications: $error');
    throw FetchDataException(message: error.toString());
  }
}
